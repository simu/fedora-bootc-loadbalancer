...
MY_IP=$(yq -r '.my_ip' /etc/floaty/fake.yaml)
NHMGR_API=$(yq -r '.next_hop_manager' /etc/floaty/fake.yaml)
DEST_ADDRS=$(yq -r '.managed-addresses|join(" ")' /etc/floaty/global.yaml)
echo "Using $MY_IP as next hop for $DEST_ADDRS" | logger -t fake-floaty
echo "Using next-hop-manager at $NHMGR_API" |logger -t fake-floaty

FIFO=$1
[[ -z $FIFO ]] && echo "A FIFO name must be specified" && exit 1
echo "Using fifo $FIFO" | logger -t fake-floaty
...
stopping()
{
	...
	echo "$PROLOGUE" STOPPING | logger -t fake-floaty
	exit 0
}
...
if [[ ! -p $FIFO ]]; then
	...
	if [[ $? -eq 0 ]]; then
		...
		echo "Unable to create fifo $FIFO" | logger -t fake-floaty
		exit 1
	fi
fi
...
[[ $? -ne 0 ]] && echo FIFO $FIFO not accessible | logger -t fake-floaty && exit 1

while [[ $SHUTDOWN -eq 0 ]]
do
	[[ ! -p $FIFO ]] && echo FIFO $FIFO missing | logger -t fake-floaty && exit 1
	exec <$FIFO
	...
	while read line; do
		...
		if [[ $TYPE = INSTANCE || $TYPE = GROUP ]]; then
			...
			echo "$PROLOGUE" $VRRP_INST $STATE $PRIORITY | logger -t fake-floaty
			if [ $STATE = "MASTER" ]; then
				for dest in $DEST_ADDRS; do
					curl "$NHMGR_API/update" --json "{\"dest\": \"$dest\", \"next_hop\":\"$MY_IP\"}" | logger -t fake-floaty
					echo "$PROLOGUE" "Updated next hop for $dest to $MY_IP" | logger -t fake-floaty
				done
			fi
		elif [[ $TYPE = VS ]]; then
			VS=$2
			...
			# Now take whatever action is required
			echo "$PROLOGUE" $TYPE $VS $STATE | logger -t fake-floaty
		elif [[ $TYPE = RS ]]; then
			RS=$2
			VS=$3
			STATE=$4
			...
			# Now take whatever action is required
			echo "$PROLOGUE" $TYPE $RS $VS $STATE | logger -t floaty
		else
			echo "$PROLOGUE" $TYPE - unknown "($*)"  | logger -t floaty
		fi
	done

	[[ $SHUTDOWN -eq 0 ]] && echo "$PROLOGUE" FIFO CLOSED |logger -t floaty
done
